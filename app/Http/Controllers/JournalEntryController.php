...
class JournalEntryController extends Controller
{
    ...
    public function create()
    {
        ...
        {
            $accounts = ChartOfAccount::select(\DB::raw('CONCAT(code, " - ", name) AS code_name, id'))->where('created_by', \Auth::user()->creatorId())->orderBy('code', 'asc')->get()->pluck('code_name', 'id');
            $accounts->prepend('--', '');
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create invoice'))
        {
            ...
            for($i = 0; $i < count($accounts); $i++)
            {
                ...
                $data[] = array(
                    'akun' => $name,
                    'deskripsi' => $accounts[$i]['description'],
                    'debit'     => isset($accounts[$i]['debit']) ? $accounts[$i]['debit'] : 0,
                    'credit' => isset($accounts[$i]['credit']) ? $accounts[$i]['credit'] : 0
                );
            }
            
            $formatdate = date('d/m/Y', strtotime($request->date));
            $response = Http::withHeaders([
                'Content-Type' => 'application/json',
            ])->post('https://uniquip.io/staging/api/journal/store', [
                'tanggal_journal' =>  $formatdate,
                'referensi_journal' => $request->reference,
                'deskripsi_journal' =>$request->description,
                'detail' => $data
            ]);
    ...
    public function update(Request $request, JournalEntry $journalEntry)
    {
        if(\Auth::user()->can('edit journal entry'))
        {
            if($journalEntry->created_by == \Auth::user()->creatorId())
            {
                ...
                $response = Http::get('https://uniquip.io/staging/api/journal');
                $result = array_search($request->reference, array_column($response['data'], 'referensi_journal')); 
                $value = @$response['data'][$result]['id'];
                $formatdate = date('d/m/Y', strtotime($request->date));
                $response = Http::put('https://uniquip.io/staging/api/journal/update/'.$value, [
                    'tanggal_journal' =>  $formatdate,
                    'referensi_journal' => $request->reference,
                    'deskripsi_journal' =>$request->description,
                    'detail' => $data
    ...
    public function destroy(JournalEntry $journalEntry)
    {
        if(\Auth::user()->can('delete journal entry'))
        {
            if($journalEntry->created_by == \Auth::user()->creatorId())
            {
                ...
                $response = Http::get('https://uniquip.io/staging/api/journal');
                $result = array_search($journalEntry->id, array_column($response['data'], 'referensi_journal')); 
                $value = @$response['data'][$result]['id'];

                $response = Http::get('https://uniquip.io/staging/api/journal/delete/'.$value);
